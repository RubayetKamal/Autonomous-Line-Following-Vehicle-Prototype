...
void turn180() {
  ...
  delay(2000);  // Adjust this delay based on your motor speed and desired turn time
  stop();
  delay(10);
}
...
void goStraightShort() {
  forward(baseSpeed);
  delay(1000);  // Adjust the delay to move a short distance
  stop();
  delay(1500);
}
...
void overtake() {
  ...
  // if(distanceFromObject>=10 && distanceFromObject<300){
  //   stop();
  //   delay(10);
  //   turnRight90();
  //   goStraightLong();
...
void avoidObject() {
  ...
  if (distanceFromObject >=10. && distanceFromObject<=50.) {
    stop();
    delay(10);
    evaluatingColour();
